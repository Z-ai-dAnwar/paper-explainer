  ...
  {eyebrow:'The overlap',title:'Numbers overlap',caption:'Measure just <b>how fast</b> the flick went? On average PSP is slower - but the ranges <b>overlap</b>. One patient\'s number can\'t call it.',
   build:function(PE){var d=PE.mkCanvas(),x=d.x,w=d.w,h=d.h;
     ...
     function draw(t){x.clearRect(0,0,w,h);PE.grid(x,w,h);var base=h*0.74,px0=w*0.12,pw=w*0.76,peak=h*0.42;
       ...
       x.save();x.setLineDash([5,5]);x.strokeStyle='rgba(255,255,255,'+(0.3+0.3*a)+')';x.beginPath();x.moveTo(qx,h*0.14);x.lineTo(qx,base);x.stroke();x.restore();
       x.save();x.fillStyle='#eef2f7';x.shadowColor='#fff';x.shadowBlur=12*a;x.beginPath();x.arc(qx,base-peak*0.55,7,0,7);x.fill();x.restore();
       x.save();x.fillStyle='#eef2f7';x.font='italic 15px '+PE.serif;x.textAlign='center';x.fillText('this patient = ?',qx,h*0.08);
